...
- Web 预览字体：本机到 `fonts.gstatic.com` 不可达，CanvasKit 在线拉 Noto Sans SC 会无限重试导致预览整页空白。字体已放 `web/fonts/`（不入库，Android 构建不包含该目录），由 `lib/web_fonts.dart` 启动时经 FontLoader 注册；新 clone 后先跑 `python tool/fetch_web_fonts.py`（jsdelivr 下载，约 25MB）。
